...
##################
	...
	def getDBdict(self, cnmtname, json=False, trans=True, content_number=False, mcstring=False, mGame=False):
		...
		try:
			...
			(
				titleid, titleversion, base_ID, keygeneration, rightsId, RSV, RGV, ctype,
				metasdkversion, exesdkversion, hasHtmlManual, Installedsize, DeltaSize, ncadata
			) = self.get_data_from_cnmt(cnmtname)
			Print.info(f"Extracted data from cnmt: titleid={titleid}, ctype={ctype}, InstalledSize={Installedsize}")
		except Exception as e:
		...
		try:
			...
			try:
				...
				Print.info(f"Retrieved names from DB: sqname={sqname}, sqeditor={sqeditor}, languages={SupLg}")
			except Exception as e:
				Print.warning(f"DB_get_names failed, trying DB_get_names_from_nutdb: {e}")
				sqname, sqeditor, SupLg = self.DB_get_names_from_nutdb(titleid)
				Print.info(f"Retrieved names from nutdb: sqname={sqname}, sqeditor={sqeditor}")
			...
			(
				nsuId, worldreleasedate, genretags, ratingtags, numberOfPlayers, intro,
				description, iconUrl, screenshots, bannerUrl, region, rating, developer,
				productCode, OnlinePlay, SaveDataCloud, playmodes, video, shopurl
			) = data
			
			# Log the retrieved content data
			Print.info(f"Content data retrieved: nsuId={nsuId}, worldreleasedate={worldreleasedate}, genretags={genretags}")
	...
	def get_data_from_cnmt(self, cnmtname):
		...
		for nca in self:
			...
			if type(nca) == Nca:
				print(f"NCA is of type Nca, checking contentType: {nca.header.contentType}")
						...
						try:
							...
							for f in nca:
								for cnmt in f:
									...
									content_entries = cnmt.readInt16()
									meta_entries = cnmt.readInt16()
									print(f"offset: {offset}, content_entries: {content_entries}, meta_entries: {meta_entries}")
									...
									min_sversion = cnmt.readInt32()
									length_of_emeta = cnmt.readInt32()
									print(f"min_sversion: {min_sversion}, length_of_emeta: {length_of_emeta}")
									...
									programSDKversion, dataSDKversion = self.getsdkvertit(titleid2)
									exesdkversion = dataSDKversion if content_type_cnmt == 'AddOnContent' else programSDKversion
									print(f"metasdkversion: {metasdkversion}, exesdkversion: {exesdkversion}")
