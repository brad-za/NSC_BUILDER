...
class Nsp(Pfs0):
    ...
    def loadCsv(
        self, line, map=["id", "path", "version", "timestamp", "hasValidTicket"]
    ):
        split = line.split("|")
    ...
    def isUpdateAvailable(self):
        ...
        if (
            self.titleId
            and title.version != None
            and self.version < title.version
            and str(title.version) != "0"
        ):
            return {
                "id": title.id,
                "baseId": title.baseId,
    ...
    def readMeta(self):
        ...
        try:
            ...
            t = self.ticket()
            rightsId = (
                hx(t.getRightsId().to_bytes(0x10, byteorder="big"))
                .decode("utf-8")
                .upper()
            )
    ...
    def open(self, path=None, mode="rb", cryptoType=-1, cryptoKey=-1, cryptoCounter=-1):
        super(Nsp, self).open(
            path or self.path, mode, cryptoType, cryptoKey, cryptoCounter
        )
    ...
    def move(self):
        ...
        if (
            os.path.abspath(self.fileName()).lower()
            == os.path.abspath(self.path).lower()
        ):
            return False
    ...
    def fileName(self):
        ...
        if not self.titleId in Titles.keys():
            if not Title.getBaseId(self.titleId) in Titles.keys():
                Print.info(
                    "could not find title key for "
                    + self.titleId
                    + " or "
                    + Title.getBaseId(self.titleId)
                )
                return None
            ...
            if type(nca) == Nca:
                if str(nca.header.contentType) == "Content.CONTROL":
                    if item == False or nca._path == item:
                        ...
                        print("-------------------------------------------------")
                        oldhash = nca.header.get_hblock_hash()
                        print("- Old nca sblock hash: " + str(hx(oldhash)))
                        newhash = nca.header.calculate_hblock_hash()
                        print("- New nca sblock hash: " + str(hx(newhash)))
    ...
    def extract_all(self, ofolder, buffer):
        ...
        for file in self:
            if not str(file._path).endswith(".ncz"):
                ...
                t = tqdm(
                    total=ncztype.header.size, unit="B", unit_scale=True, leave=False
                )
                t.write(tabs + "Copying: " + str(filename))
                # print(file._path)
                ncztype.rewind()
                ...
                with open(filepath, "wb+") as o:
                    ...
                    curr_off = o.tell()
                    t.write(
                        tabs + "- Appending decompressed {}".format(str(ncztype._path))
                    )
                    ...
                    for s in sections:
                        ...
                        if s.cryptoType == 1:  # plain text
                            t.write(
                                tabs + "    * Section {} is plaintext".format(str(c))
                            )
                            t.write(
                                tabs
                                + "      %x - %d bytes, Crypto type %d"
                            ...
                            t.write(
                                tabs
                                + "    * Section {} needs decompression".format(str(c))
                            )
                            t.write(
                                tabs
                                + "      %x - %d bytes, Crypto type %d"
            ...
            if type(nca) == Nca:
                ...
                if nca.header.getRightsId() != 0:
                    ...
                    if correct == True:
                        ...
                        tk = Keys.decryptTitleKey(
                            tkey, Keys.getMasterKeyIndex(int(masterKeyRev))
                        )
                else:
                    ...
                    try:
                        ...
                        if nca.size < int(1073725440):
                            try:
                                ...
                                nca3.decrypt_to_plaintext(
                                    PN.replace(str(nca._path)[lon:], ext)
                                )
                            except BaseException as e:
                                # Print.error('Exception: ' + str(e))
                                try:
                                    ...
                                    t.write(
                                        str(nca._path) + " needs to be pre-extracted"
                                    )
                                    outnca = open(str(PN), "w+b")
                                    nca.rewind()
                                    try:
                                        for data in iter(
                                            lambda: nca.read(int(buffer)), ""
                                        ):
                                            outnca.write(data)
                                            t.update(len(data))
                                            outnca.flush()
                                        ...
                                        with open(str(PN), "rb") as f:
                                            nca3 = NCA3(
                                                f, int(0), str(nca._path), tk, buffer
                                            )
                                            nca3.decrypt_to_plaintext(
                                                PN.replace(str(nca._path)[lon:], ext)
                                            )
                            ...
                            t = tqdm(
                                total=nca.size, unit="B", unit_scale=True, leave=False
                            )
                ...
                if str(nca.header.contentType) == "Content.PROGRAM":
                    ...
                    try:
                        ...
                        for fs in nca.sectionFilesystems:
                            ...
                            # print(fs.cryptoType)
                            if (
                                fs.fsType == Type.Fs.PFS0
                                and fs.cryptoType == Type.Crypto.CTR
                            ):
                                nca.seek(0)
                                ncaHeader = NcaHeader()
                                ncaHeader.open(
                                ...
                                for i in range(n_files):
                                    ...
                                    nameOffset = int.from_bytes(
                                        nameOffset, byteorder="little"
                                    )
                                    name = (
                                ...
                                for i in range(len(files_list)):
                                    if files_list[i][0] == "main.npdm":
                                        off1 = (
                                            files_list[i][1] + pfs0Offset + headerSize
                                        )
                                        nca.seek(off1)
        ...
        for nca in self:
            ...
            if type(nca) == Nca:
                if str(nca.header.contentType) == "Content.META":
                    for f in nca:
                        for cnmt in f:
                            ...
                            if content_type_cnmt == "Patch":
                                ...
                                tit_name, editor, ediver, SupLg, regionstr, isdemo = (
                                    self.inf_get_title(
                                        target, offset, content_entries, original_ID
                                    )
                                )
                                if tit_name == "DLC":
                                    tit_name = "-"
                            ...
                            original_ID2 = str(
                                hx(original_ID.to_bytes(8, byteorder="big"))
                            )
                            original_ID2 = original_ID2[2:-1]
                            version = str(
                                int.from_bytes(titleversion, byteorder="little")
                            )
                            ...
                            if crypto1 == 2:
                                ...
                                keygen = nca.header.getCryptoType2()
                            programSDKversion, dataSDKversion = self.getsdkvertit(
                                titleid2
                            )
                            ...
                            print(message)
                            feed += message + "\n"
                            message = "CONTENT ID: " + str(titleid2)
                            print(message)
                            feed += message + "\n"
                            message = "-----------------------------"
                            print(message)
                            ...
                            if content_type_cnmt != "AddOnContent":
                                ...
                                print(message)
                                feed += message + "\n"
                                message = "- Name: " + tit_name
                                print(message)
                                feed += message + "\n"
                                message = "- Editor: " + editor
                                print(message)
                                feed += message + "\n"
                                message = "- Display Version: " + str(ediver)
                                print(message)
                                feed += message + "\n"
                                message = "- Meta SDK version: " + sdkversion
                                print(message)
                                feed += message + "\n"
                                message = "- Program SDK version: " + programSDKversion
                                print(message)
                                feed += message + "\n"
                            ...
                            if content_type_cnmt == "AddOnContent":
                                ...
                                print(message)
                                feed += message + "\n"
                                message = "- Data SDK version: " + dataSDKversion
                                print(message)
                                feed += message + "\n"
                                if SupLg != "":
                                    suplangue = str((", ".join(SupLg)))
                            ...
                            if content_type_cnmt == "AddOnContent":
                                ...
                                message = reqtag + str(min_sversion) + " -> " + RSV_rq
                                print(message)
                                feed += message + "\n"
                            message = (
                            ...
                            for i in range(content_entries):
                                ...
                                IdOffset = int.from_bytes(
                                    IdOffset, byteorder="little", signed=True
                                )
                                ...
                                if ncatype != 6:
                                    ...
                                    s1 = 0
                                    s1, feed = self.print_nca_by_title(
                                        nca_name, ncatype, showID, feed
                                    )
                                    ncasize = ncasize + s1
                            ...
                            print(message)
                            feed += message + "\n"
                            message = bigtab + "  TOTAL SIZE: " + size_pr
                            print(message)
                            feed += message + "\n"
                            if self.actually_has_deltas(ncalist) == "true":
                                ...
                                for i in range(content_entries):
                                    ...
                                    size = cnmt.read(0x6)
                                    ncatype = cnmt.read(0x1)
                                    ncatype = int.from_bytes(
                                        ncatype, byteorder="little"
                                    )
                                    IdOffset = cnmt.read(0x1)
                                    IdOffset = int.from_bytes(
                                        IdOffset, byteorder="little", signed=True
                            ...
                            if self.actually_has_other(titleid2, ncalist) == "true":
                                ...
                                os2 = 0
                                os3 = 0
                                os1, feed = self.print_xml_by_title(
                                    ncalist, contentlist, feed
                                )
                                os2, feed = self.print_tac_by_title(
                                    titleid2, contentlist, feed
                                )
                                os3, feed = self.print_jpg_by_title(
                                    ncalist, contentlist, feed
                                )
                    ...
                    print(message)
                    feed += message + "\n"
                    message = "   FULL CONTENT TOTAL SIZE: " + size_pr + "   "
                    print(message)
                    feed += message + "\n"
                    message = "/////////////////////////////////////"
                    print(message)
        ...
        for nca in self:
            ...
            if type(nca) == Nca:
                if filename == nca_name:
                    ...
                    if ncatype != "Meta: ":
                        message = (
                            f"- {ncatype}{tab}{str(filename)}{tab}{tab}Size: {size_pr}"
                        )
                        print(message)
                        feed += message + "\n"
                    else:
                        message = f"- {ncatype}{tab}{str(filename)}{tab}Size: {size_pr}"
                        print(message)
                        feed += message + "\n"
                    # if ncatype != "Meta: ":
                ...
                if ncatype != "Meta: ":
                    ...
                    message = (
                        "- " + ncatype + tab + str(filename) + tab + "Size: " + size_pr
                    )
                    print(message)
                    feed += message + "\n"
                return size, feed
    ...
    def print_xml_by_title(self, ncalist, contentlist, feed=""):
        ...
        for file in self:
            if file._path.endswith(".xml"):
                ...
                if xml in ncalist:
                    message = (
                        "- XML: " + tab * 2 + str(filename) + tab + "Size: " + size_pr
                    )
                    print(message)
                    feed += message + "\n"
    ...
    def print_tac_by_title(self, titleid, contentlist, feed=""):
        ...
        for cert in self:
            if cert._path.endswith(".cert"):
                ...
                if cert_id == titleid:
                    message = (
                        "- Cert: " + tab + str(filename) + tab * 2 + "Size: " + size_pr
                    )
                    print(message)
                    feed += message + "\n"
                    contentlist.append(filename)
    ...
    def printnonlisted(self, contentlist, feed=""):
        ...
        if list_nonlisted == "true":
            ...
            print(message)
            feed += message + "\n"
            message = "FILES NOT LINKED TO CONTENT IN NSP"
            print(message)
            feed += message + "\n"
            message = "-----------------------------------"
            print(message)
            ...
            for file in self:
                ...
                if not filename in contentlist and not nczname in contentlist:
                    ...
                    message = str(filename) + 3 * tab + "Size: " + size_pr
                    print(message)
                    feed += message + "\n"
            bigtab = "\t" * 7
    ...
    def adv_content_list(self):
                        ...
                        for cnmt in f:
                            ...
                            if content_type_cnmt == "Application":
                                ...
                                if tit_name == "DLC":
                                    ...
                                    editor = "-"
                                applist.append(
                                    [target, titleid, version, tit_name, editor]
                                )
                            if content_type_cnmt == "Patch":
                                patchlist.append(
                                    [target, original_ID, version, titleid]
                                )
                            ...
                            if content_type_cnmt == "AddOnContent":
                                ...
                                DLCnumb = int(DLCnumb)
                                dlclist.append(
                                    [target, original_ID, version, titleid, DLCnumb]
                                )
        ...
        if len(applist) != 0:
            for i in range(len(applist)):
                ...
                print(message)
                feed += message + "\n"
                message = "BASE CONTENT ID: " + str(tid)
                print(message)
                feed += message + "\n"
                message = "------------------------------------------------"
                print(message)
                feed += message + "\n"
                message = "Name: " + applist[i][3]
                print(message)
                feed += message + "\n"
                message = "Editor: " + applist[i][4]
                print(message)
                feed += message + "\n"
                message = "------------------------------------------------"
                print(message)
                feed += message + "\n"
                message = applist[i][1] + " [BASE]" + " v" + applist[i][2]
                print(message)
                feed += message + "\n"
                cupd = 0
            ...
            message = "This option is currently meant for multicontent, that includes at least a base game"
            print(message)
            feed += message + "\n"
        return feed
            ...
            if type(nca) == Nca:
                if str(nca.header.contentType) == "Content.CONTROL":
                    ...
                    for f in nca:
                        f.seek(offset)
                        nacp = Nacp()
                        feed = nacp.par_getNameandPub(
                            f.read(0x300 * 15), feed, gui, roma
                        )
                        if gui == True:
                            message = "..............................."
                            print(message)
                        ...
                        feed = nacp.par_getStartupUserAccount(
                            f.readInt8("little"), feed
                        )
                        feed = nacp.par_getUserAccountSwitchLock(
                            f.readInt8("little"), feed
                        )
                        feed = nacp.par_getAddOnContentRegistrationType(
                            f.readInt8("little"), feed
                        )
                        ...
                        try:
                            ...
                            feed = nacp.par_getAddOnContentBaseId(
                                f.readInt64("little"), feed
                            )
                            f.seek(offset + 0x3078)
                            feed = nacp.par_getSaveDataOwnerId(
                                f.readInt64("little"), feed
                            )
                            f.seek(offset + 0x3080)
                            feed = nacp.par_getUserAccountSaveDataSize(
                                f.readInt64("little"), feed
                            )
                            f.seek(offset + 0x3088)
                            feed = nacp.par_getUserAccountSaveDataJournalSize(
                                f.readInt64("little"), feed
                            )
                            f.seek(offset + 0x3090)
                            feed = nacp.par_getDeviceSaveDataSize(
                                f.readInt64("little"), feed
                            )
                            f.seek(offset + 0x3098)
                            feed = nacp.par_getDeviceSaveDataJournalSize(
                                f.readInt64("little"), feed
                            )
                            f.seek(offset + 0x30A0)
                            feed = nacp.par_getBcatDeliveryCacheStorageSize(
                                f.readInt64("little"), feed
                            )
                            f.seek(offset + 0x30A8)
                            feed = nacp.par_getApplicationErrorCodeCategory(
                                f.read(0x07), feed
                            )
                            f.seek(offset + 0x30B0)
                            feed = nacp.par_getLocalCommunicationId(
                                f.readInt64("little"), feed
                            )
                            f.seek(offset + 0x30F0)
                            feed = nacp.par_getLogoType(f.readInt8("little"), feed)
                            feed = nacp.par_getLogoHandling(f.readInt8("little"), feed)
                            feed = nacp.par_getRuntimeAddOnContentInstall(
                                f.readInt8("little"), feed
                            )
                            f.seek(offset + 0x30F6)
                            feed = nacp.par_getCrashReport(f.readInt8("little"), feed)
                            feed = nacp.par_getHdcp(f.readInt8("little"), feed)
                            feed = nacp.par_getSeedForPseudoDeviceId(
                                f.readInt64("little"), feed
                            )
                            f.seek(offset + 0x3100)
                            feed = nacp.par_getBcatPassphrase(f.read(0x40), feed)
                            f.seek(offset + 0x3148)
                            feed = nacp.par_UserAccountSaveDataSizeMax(
                                f.readInt64("little"), feed
                            )
                            f.seek(offset + 0x3150)
                            feed = nacp.par_UserAccountSaveDataJournalSizeMax(
                                f.readInt64("little"), feed
                            )
                            f.seek(offset + 0x3158)
                            feed = nacp.par_getDeviceSaveDataSizeMax(
                                f.readInt64("little"), feed
                            )
                            f.seek(offset + 0x3160)
                            feed = nacp.par_getDeviceSaveDataJournalSizeMax(
                                f.readInt64("little"), feed
                            )
                            f.seek(offset + 0x3168)
                            feed = nacp.par_getTemporaryStorageSize(
                                f.readInt64("little"), feed
                            )
                            feed = nacp.par_getCacheStorageSize(
                                f.readInt64("little"), feed
                            )
                            f.seek(offset + 0x3178)
                            feed = nacp.par_getCacheStorageJournalSize(
                                f.readInt64("little"), feed
                            )
                            feed = nacp.par_getCacheStorageDataAndJournalSizeMax(
                                f.readInt64("little"), feed
                            )
                            f.seek(offset + 0x3188)
                            feed = nacp.par_getCacheStorageIndexMax(
                                f.readInt64("little"), feed
                            )
                            f.seek(offset + 0x3188)
                            feed = nacp.par_getPlayLogQueryableApplicationId(
                                f.readInt64("little"), feed
                            )
                            f.seek(offset + 0x3210)
                            feed = nacp.par_getPlayLogQueryCapability(
                                f.readInt8("little"), feed
                            )
                            feed = nacp.par_getRepair(f.readInt8("little"), feed)
                            feed = nacp.par_getProgramIndex(f.readInt8("little"), feed)
                            feed = nacp.par_getRequiredNetworkServiceLicenseOnLaunch(
                                f.readInt8("little"), feed
                            )
                        except:
                            continue
                ...
                if str(nca.header.contentType) == "Content.META":
                    for f in nca:
                        for cnmt in f:
                            ...
                            print(message)
                            feed += message + "\n"
                            message = "Reading: " + str(cnmt._path)
                            print(message)
                            feed += message + "\n"
                            message = "..........................................."
                            print(message)
                            ...
                            message = (
                                "Titleid = "
                                + (
                                    str(hx(titleid.to_bytes(8, byteorder="big")))[2:-1]
                                ).upper()
                            )
                            print(message)
                            feed += message + "\n"
                            ...
                            print(message)
                            feed += message + "\n"
                            message = "Table offset = " + str(
                                hx((offset + 0x20).to_bytes(2, byteorder="big"))
                            )
                            print(message)
                            feed += message + "\n"
                            message = "Number of content = " + str(content_entries)
                            print(message)
                            feed += message + "\n"
                            message = "Number of meta entries = " + str(meta_entries)
                            print(message)
                            feed += message + "\n"
                            message = (
                            ...
                            if content_name == "AddOnContent":
                                message = "RequiredUpdateNumber = " + str(min_sversion)
                                print(message)
                                feed += message + "\n"
                            if content_name != "AddOnContent":
                                message = "RequiredSystemVersion = " + str(min_sversion)
                                print(message)
                                feed += message + "\n"
                            message = "Length of exmeta = " + str(length_of_emeta)
                            print(message)
                            feed += message + "\n"
                            cnmt.rewind()
                            cnmt.seek(0x20 + offset)
                            for i in range(content_entries):
                                ...
                                print(message)
                                feed += message + "\n"
                                message = "Content number " + str(i + 1)
                                print(message)
                                feed += message + "\n"
                                message = "........................"
                                print(message)
                                feed += message + "\n"
                                vhash = cnmt.read(0x20)
                                message = "Hash =\t" + str(hx(vhash))
                                print(message)
                                feed += message + "\n"
                                NcaId = cnmt.read(0x10)
                                message = "NcaId =\t" + str(hx(NcaId))
                                print(message)
                                feed += message + "\n"
                                size = cnmt.read(0x6)
                                message = "Size =\t" + str(
                                    sq_tools.getSize(
                                        int.from_bytes(
                                            size, byteorder="little", signed=True
                                        )
                                    )
                                )
                                print(message)
                                feed += message + "\n"
                                ncatype = cnmt.read(0x1)
                                message = "Ncatype = " + sq_tools.getmetacontenttype(
                                    str(
                                        int.from_bytes(
                                            ncatype, byteorder="little", signed=True
                                        )
                                    )
                                )
                                print(message)
                                ...
                                feed += message + "\n"
                            cnmt.seek(
                                0x20 + offset + content_entries * 0x38 + length_of_emeta
                            )
                            ...
                            if length_of_emeta > 0:
                                ...
                                cnmt.read(0x4)
                                message = "Number of previous cnmt entries = " + str(
                                    int.from_bytes(num_prev_cnmt, byteorder="little")
                                )
                                print(message)
                                feed += message + "\n"
                                message = "Number of previous delta entries = " + str(
                                    int.from_bytes(num_prev_delta, byteorder="little")
                                )
                                print(message)
                                feed += message + "\n"
                                message = "Number of delta info entries = " + str(
                                    int.from_bytes(num_delta_info, byteorder="little")
                                )
                                print(message)
                                feed += message + "\n"
                                message = "Number of previous content entries = " + str(
                                    int.from_bytes(
                                        num_previous_content, byteorder="little"
                                    )
                                )
                                print(message)
                                feed += message + "\n"
                                message = "Number of delta content entries = " + str(
                                    int.from_bytes(
                                        num_delta_content, byteorder="little"
                                    )
                                )
                                print(message)
                                feed += message + "\n"
                                for i in range(
                                    ...
                                    print(message)
                                    feed += message + "\n"
                                    message = "Previous cnmt records: " + str(i + 1)
                                    print(message)
                                    feed += message + "\n"
                                    message = (
                                        "..........................................."
                                    ...
                                    print(message)
                                    feed += message + "\n"
                                    message = "Content Type = " + sq_tools.cnmt_type(
                                        type_n
                                    )
                                    print(message)
                                    feed += message + "\n"
                                    ...
                                    message = (
                                        "Ncatype = "
                                        + sq_tools.getmetacontenttype(
                                            str(
                                                int.from_bytes(
                                                    unknown2, byteorder="little"
                                                )
                                            )
                                        )
                                    )
                                ...
                                for i in range(
                                    ...
                                    print(message)
                                    feed += message + "\n"
                                    message = "Previous delta records: " + str(i + 1)
                                    print(message)
                                    feed += message + "\n"
                                    message = (
                                        "..........................................."
                                    ...
                                    message = "Old titleid = " + str(
                                        hx(oldtitleid.to_bytes(8, byteorder="big"))
                                    )
                                    print(message)
                                    feed += message + "\n"
                                    message = "New titleid = " + str(
                                        hx(newtitleid.to_bytes(8, byteorder="big"))
                                    )
                                    print(message)
                                    feed += message + "\n"
                                    message = "Old version = " + str(
                                        int.from_bytes(
                                            oldtitleversion, byteorder="little"
                                        )
                                    )
                                    print(message)
                                    feed += message + "\n"
                                    message = "New version = " + str(
                                        int.from_bytes(
                                            newtitleversion, byteorder="little"
                                        )
                                    )
                                    print(message)
                                    feed += message + "\n"
                                ...
                                for i in range(
                                    ...
                                    feed += message + "\n"
                                    message = "Index1 = " + str(
                                        hx(index1.to_bytes(8, byteorder="big"))
                                    )
                                    print(message)
                                    feed += message + "\n"
                                    message = "Index2 = " + str(
                                        hx(index2.to_bytes(8, byteorder="big"))
                                    )
                                    print(message)
                                    feed += message + "\n"
                                    # Print.info('unknown1 = ' + str(int.from_bytes(unknown1, byteorder='little')))
                                for i in range(
                                    int.from_bytes(
                                        num_delta_application, byteorder="little"
                                    )
                                ):
                                    message = (
                                        "..........................................."
                                    ...
                                    print(message)
                                    feed += message + "\n"
                                    message = "Delta application info: " + str(i + 1)
                                    print(message)
                                    feed += message + "\n"
                                    message = (
                                        "..........................................."
                                    ...
                                    message = "OldNcaId = " + str(hx(OldNcaId))
                                    print(message)
                                    feed += message + "\n"
                                    message = "NewNcaId = " + str(hx(NewNcaId))
                                    print(message)
                                    feed += message + "\n"
                                    message = "Old size = " + str(
                                        sq_tools.getSize(
                                            int.from_bytes(
                                                old_size,
                                                byteorder="little",
                                    ...
                                    print(message)
                                    feed += message + "\n"
                                    message = "Unknown1 = " + str(
                                        int.from_bytes(unknown1, byteorder="little")
                                    )
                                    print(message)
                                    feed += message + "\n"
                                    message = (
                                    ...
                                    print(message)
                                    feed += message + "\n"
                                    message = "Installable = " + str(
                                        int.from_bytes(installable, byteorder="little")
                                    )
                                    print(message)
                                    feed += message + "\n"
                                    message = "Upper 2 bytes of the new size=" + str(
                                        hx(up2bytes)
                                    )
                                    print(message)
                                    feed += message + "\n"
                                    message = "Lower 4 bytes of the new size=" + str(
                                        hx(low4bytes)
                                    )
                                    print(message)
                                    feed += message + "\n"
                                for i in range(
                                    ...
                                    print(message)
                                    feed += message + "\n"
                                    message = "Previous content records: " + str(i + 1)
                                    print(message)
                                    feed += message + "\n"
                                    message = (
                                        "..........................................."
                                ...
                                for i in range(
                                    ...
                                    print(message)
                                    feed += message + "\n"
                                    message = "Delta content entry " + str(i + 1)
                                    print(message)
                                    feed += message + "\n"
                                    message = (
                                        "..........................................."
    ...
    def ret_xml(self, nca):
        ...
        for f in nca:
            for cnmt in f:
                ...
                xml_string += (
                    "  <KeyGeneration>" + str(keygeneration) + "</KeyGeneration>" + "\n"
                )
                xml_string += (
        ...
        for nca in self:
            if type(nca) == Nca:
                ...
                if nca.header.getRightsId() != 0:
                    ...
                    nca.rewind()
                    t = tqdm(
                        total=nca.header.size, unit="B", unit_scale=True, leave=False
                    )
                    t.write(tabs + "Copying: " + str(filename))
                    for data in iter(lambda: nca.read(int(buffer)), ""):
                        fp.write(data)
                            ...
                            for pfs0 in nca:
                                for cnmt in pfs0:
                                    ...
                                    if check == "AddOnContent":
                                        ...
                                        Print.info(
                                            tabs + "DLC -> No need to patch the meta"
                                        )
                                        Print.info(
                                            tabs
                                            + "-------------------------------------"
                ...
                if str(nca.header.contentType) == "Content.META":
                    for f in nca:
                        for cnmt in f:
                            ...
                            target = str(nca._path)
                            contentname = self.splitter_get_title(
                                target, offset, content_entries, original_ID
                            )
                            cnmt.rewind()
                            cnmt.seek(0x20 + offset)
                    ...
                    for f in nca:
                        for cnmt in f:
                            ...
                            (
                                ...
                                video,
                                shopurl,
                            ) = nutdb.get_content_data(titleid2, trans)
                            sdkversion = nca.get_sdkversion()
                            message = "-----------------------------"
                            print(message)
                            feed += message + "\n"
                            ...
                            if content_type_cnmt == "AddOnContent":
                                ...
                                message = reqtag + str(RSversion) + " -> " + RSV_rq
                                print(message)
                                feed += message + "\n"
                            message = (
                                "- Encryption (keygeneration): "
                            ...
                            if content_type_cnmt != "AddOnContent":
                                ...
                                feed += message + "\n"
                            try:
                                if content_type_cnmt != "AddOnContent" and not str(
                                    self.path
                                ).endswith(".nsz"):
                                    message = "ExeFS Data:"
                                    print(message)
                                    ...
                                    message = "- BuildID8: " + BuildID8
                                    print(message)
                                    feed += message + "\n"
                                    message = "- BuildID:  " + sq_tools.trimm_module_id(
                                        ModuleId
                                    )
                                    print(message)
                                    feed += message + "\n"
                            ...
                            if (
                                nsuId != False
                                or numberOfPlayers != False
                                or releaseDate != False
                                or category != False
                                or ratingContent != False
                            ):
                                message = "Eshop Data:"
                                print(message)
                                feed += message + "\n"
                            if nsuId != False:
                                ...
                                feed += message + "\n"
                            if region != False:
                                message = "- Data from Region: " + region
                                print(message)
                                feed += message + "\n"
                            if numberOfPlayers != False:
                                message = "- Number of Players: " + numberOfPlayers
                                print(message)
                                feed += message + "\n"
                        ...
                        for f in nca:
                            for cnmt in f:
                                ...
                                if testID == original_ID:
                                    ...
                                    DLCnumb = bytes.fromhex(DLCnumb)
                                    DLCnumb = str(
                                        int.from_bytes(DLCnumb, byteorder="big")
                                    )
                                    DLCnumb = int(DLCnumb)
    ...
    def updbase_copy(
        self, ofolder, buffer, nca_name, metapatch, keypatch, RSV_cap, titleKeyDec
    ):
        indent = 1
        tabs = "\t" * indent
        ...
        if type(nca) == Nca:
            if nca.header.getCryptoType2() != newMasterKeyRev:
                if silent == False:
                    ...
                    Print.info(
                        tabs
                        + "Changing keygeneration from %d to %s"
                        % (nca.header.getCryptoType2(), str(newMasterKeyRev))
                    )
                    Print.info(tabs + "-----------------------------------")
                encKeyBlock = nca.header.getKeyBlock()
                if sum(encKeyBlock) != 0:
                ...
                if keypatch != "false":
                    if keypatch < target.header.getCryptoType2():
                        encKeyBlock, crypto1, crypto2 = self.get_new_cryptoblock(
                            nca, keypatch, encKeyBlock, t
                        )
                newheader = self.get_newheader(
                    target, encKeyBlock, crypto1, crypto2, hcrypto, gc_flag
                )
                target.rewind()
        ...
        for nca in self:
            ...
            if type(nca) == Nca and (str(nca.header.contentType) != "Content.META"):
                ...
                for data in iter(lambda: nca.read(int(buffer)), ""):
                    if i == 0:
                        newheader = self.get_newheader(
                            nca, encKeyBlock, crypto1, crypto2, hcrypto, gc_flag
                        )
                ...
                crypto = aes128.AESECB(
                    Keys.keyAreaKey(
                        Keys.getMasterKeyIndex(masterKeyRev), ncztype.header.keyIndex
                    )
                )
                hcrypto = aes128.AESXTS(uhx(Keys.get("header_key")))
                gc_flag = "00" * 0x01
                if ncztype.header.getRightsId() != 0:
                    ...
                    if keypatch != "false":
                        if keypatch < ncztype.header.getCryptoType2():
                            encKeyBlock, crypto1, crypto2 = self.get_new_cryptoblock(
                                ncztype, keypatch, encKeyBlock, t
                            )
                if ncztype.header.getRightsId() == 0:
                    ncztype.rewind()
                ...
                i = 0
                newheader = self.get_newheader(
                    ncztype, encKeyBlock, crypto1, crypto2, hcrypto, gc_flag
                )
                ncztype.rewind()
                ...
                if str(nca.header.contentType) == "Content.META":
                    for f in nca:
                        for cnmt in f:
                            ...
                            length_of_emeta = cnmt.readInt32()
                            target = str(nca._path)
                            titlerights, ckey = self.getdbtr(
                                original_ID, content_type, titleid2
                            )
                            if ckey == "0":
                            ...
                            tit_name, editor, ediver, SupLg, regionstr, isdemo = (
                                self.inf_get_title(
                                    target, offset, content_entries, original_ID, roman
                                )
                            )
                            if tit_name == "DLC" and (
                                str(titlerights).endswith("000")
                                or str(titlerights).endswith("800")
                            ):
                                tit_name = "-"
                                editor = "-"
                            if dbtype == "extended" or dbtype == "all":
                                dbstring = self.getdbstr(
                                    titleid2,
                                    titlerights,
                                    ckey,
                                    content_type,
                            ...
                            if dbtype == "keyless" or dbtype == "all":
                                kdbstring = self.getkeylessdbstr(
                                    titleid2,
                                    titlerights,
                                    ckey,
                                    content_type,
                ...
                if nca.header.getRightsId() != 0:
                    ...
                    if original_ID == nca_id:
                        ...
                        if check.endswith("800") and content_type == "Patch":
                            return tr, titleKeyDec
                        if (
                            not check.endswith("000")
                            and not tr.endswith("800")
                            and content_type == "AddOnContent"
                        ):
                            return tr, titleKeyDec
                    else:
                        tr = str(nca.header.rightsId)
    ...
    def getnutdbstr(
        ...
        if regionstr[0] == "1" and regionstr[2] == "0":
            ...
            Regionvar = "EUR"
        elif regionstr[4] == "1" and (
            regionstr[24] == "1" or regionstr[26] == "1" or regionstr[28] == "1"
        ):
            Regionvar = "AS"
    ...
    def rebuild(self, buffer, filepath, delta, ndskip, xml_gen):
        ...
        fileOffsets = [sum(fileSizes[:n]) for n in range(filesNb)]
        fileNamesLengths = [
            len(str(file)) + 1 for file in contentlist
        ]  # +1 for the \x00
        stringTableOffsets = [sum(fileNamesLengths[:n]) for n in range(filesNb)]
                    ...
                    for f in nca:
                        for cnmt in f:
                            ...
                            for i in range(content_entries):
                                ...
                                ncz_name = nca_name[:-1] + "z"
                                if (
                                    nca_name in completefilelist
                                    or ncz_name in completefilelist
                                ):
                                    if delta == False and ncatype == 6:
                                        print(
                                            tabs
                                            + "- Excluding delta fragment "
                                            + nca_name
                                        )
                                        continue
                                    else:
                            ...
                            if nca_meta in completefilelist:
                                ...
                                if ofolder != False:
                                    ...
                                    if vkeypatch == False:
                                        ...
                                        xml = target.xml_gen_mod(
                                            ofolder, nsha, vkeypatch
                                        )
                                    xmlname = nca_meta[:-3] + "xml"
    ...
    def append_content(
        self, outf, target, buffer, t, fat="exfat", fx="files", c=0, index=0
    ):
        block = 4294934528
        indent = 1
        tabs = "\t" * indent
            ...
            if str(file._path) == target:
                    ...
                    if file.header.getRightsId() != 0:
                        ...
                        for i in range(len(ticketlist)):
                            ...
                            if str(file.header.rightsId) == ticketlist[i][1]:
                                ...
                                break
                        titleKeyDec = Keys.decryptTitleKey(
                            encryptedkey, Keys.getMasterKeyIndex(masterKeyRev)
                        )
                        t.write("")
                        ...
                        if keypatch != "false":
                            if keypatch < file.header.getCryptoType2():
                                encKeyBlock, crypto1, crypto2 = (
                                    self.get_new_cryptoblock(
                                        file, keypatch, encKeyBlock, t
                                    )
                                )
                ...
                if ncztype.header.getRightsId() != 0:
                    ...
                    if str(ncztype.header.contentType) != "Content.META":
                        ...
                        sha = sha256()
                        t.write(
                            "- Appending decompressed {}".format(str(ncztype._path))
                        )
                        ...
                        with open(outf, "rb+") as o:
                            ...
                            for s in sections:
                                ...
                                if s.cryptoType == 1:  # plain text
                                    t.write(
                                        tabs
                                        + "    * Section {} is plaintext".format(str(c))
                                    )
                                    t.write(
                                        tabs
                                        + "      %x - %d bytes, Crypto type %d"
                                    ...
                                    while i < end:
                                        chunkSz = (
                                            buffer if end - i > buffer else end - i
                                        )
                                        chunk = reader.read(chunkSz)
                                        if not len(chunk):
                                            break
                                    ...
                                    raise IOError(
                                        "Unknown crypto type: %d" % s.cryptoType
                                    )
                                else:
                                    t.write(
                                        tabs
                                        + "    * Section {} needs decompression".format(
                                            str(c)
                                        )
                                    )
                                    t.write(
                                    ...
                                    t.write(
                                        tabs + "      Key: %s" % (str(hx(s.cryptoKey)))
                                    )
                                    t.write(
                                        tabs
                                        + "      IV: %s" % (str(hx(s.cryptoCounter)))
                                    )
                                    crypto = AESCTR(s.cryptoKey, s.cryptoCounter)
                                    spsize += s.size
                                    test = int(spsize / (buffer))
    ...
    def cd_spl_nsp(self, buffer, ofile, ofolder, filelist, fat, fx, nodecompress=False):
        ...
        for file in self:
            if type(file) == Nca and file._path in contentlist:
                ...
                crypto = aes128.AESECB(
                    Keys.keyAreaKey(
                        Keys.getMasterKeyIndex(masterKeyRev), file.header.keyIndex
                    )
                )
                hcrypto = aes128.AESXTS(uhx(Keys.get("header_key")))
                gc_flag = "00" * 0x01
                ...
                str(file._path).endswith(".ncz")
                and (str(file._path)[:-1] + "a") in contentlist
                and nodecompress == False
            ):
                ncztype = Nca(file)
                ncztype._path = file._path
                ncztype.rewind()
                ...
                str(file._path).endswith(".ncz")
                and (str(file._path)[:-1] + "z") in contentlist
                and nodecompress == True
            ):
                file.rewind()
                t.write(tabs + "- Appending: " + str(file._path))
                outf = open(str(filepath), "a+b")
    ...
    def cd_spl_xci(self, buffer, ofile, ofolder, filelist, fat, fx, nodecompress=False):
        ...
        (
            ...
            norm_multiplier,
            sec_multiplier,
        ) = self.cd_spl_gen_xcih(contentlist, nodecompress)
        totSize = (
            len(xci_header)
            + len(game_info)
            + len(sig_padding)
                ...
                if ncapath in filelist:
                    ...
                    if ncztype.header.getRightsId() != 0:
                        if ncztype.header.masterKeyRev != masterKeyRev:
                            print("WARNING!!! Mismatched masterKeyRevs!")
                            print(
                                f"{str(ncztype._path)} - {ncztype.header.masterKeyRev}"
                            )
                            print(f"{str(ticket._path)} - {masterKeyRev}")
        for file in self:
            if type(file) == Nca and file._path in filelist:
        ...
        for nca in self:
            if type(nca) == Nca and nca._path in contentlist:
                ...
                str(nca._path).endswith(".ncz")
                and (str(nca._path)[:-1] + "a") in contentlist
                and nodecompress == False
            ):
                ncztype = Nca(nca)
                ncztype._path = nca._path
                ...
                str(nca._path).endswith(".ncz")
                and (str(nca._path)[:-1] + "z") in contentlist
                and nodecompress == True
            ):
                ncztype = Nca(nca)
                ncztype._path = nca._path
    ...
    def verify(self):
        ...
        for file in listed_files:
            ...
            if file in validfiles:
                if file.endswith("cnmt.nca"):
                    for f in self:
                        if str(f._path) == file:
                            message = (
                                str(f.header.titleId)
                                + " - "
                                + str(f.header.contentType)
                            )
                            print(message)
                            feed += message + "\n"
                            for nf in f:
                    ...
                    for f in self:
                        if str(f._path)[:-1] == file[:-1]:
                            ...
                            message = (
                                str(ncztype.header.titleId)
                                + " - "
                                + str(ncztype.header.contentType)
                            )
                            print(message)
                            feed += message + "\n"
            ...
            if correct == True:
                if file.endswith("cnmt.nca"):
                    message = tabs + file + " -> is CORRECT"
                    print(message)
                    feed += message + "\n"
                else:
                    message = tabs + file + tabs + "  -> is CORRECT"
                    print(message)
                    feed += message + "\n"
            elif correct == "ncz":
                message = tabs + file + tabs + "  -> ncz file needs HASH check"
                print(message)
                feed += message + "\n"
            elif correct == "nsz":
                pass
                ...
                verdict = False
                if file.endswith("cnmt.nca"):
                    message = tabs + file + " -> is CORRUPT <<<-"
                    print(message)
                    feed += message + "\n"
                elif file.endswith("nca"):
                    message = tabs + file + tabs + "  -> is CORRUPT <<<-"
                    print(message)
                    feed += message + "\n"
                    if baddec == True:
                        message = (
                    ...
                    message = tabs + file + tabs + "  -> titlekey is INCORRECT <<<-"
                    print(message)
                    feed += message + "\n"
        for nca in self:
            if type(nca) == Nca:
                    ...
                    for f in nca:
                        for cnmt in f:
                            ...
                            for i in range(content_entries):
                                ...
                                if (nca_name not in listed_files and ncatype != 6) or (
                                    nca_name not in validfiles and ncatype != 6
                                ):
        ...
        for nca in self:
            if type(nca) == Nca:
                if nca.header.getRightsId() != 0:
                    rightsId = (
                        hx(nca.header.getRightsId().to_bytes(0x10, byteorder="big"))
                        .decode("utf-8")
                        .lower()
                    )
                    if rightsId not in titlerights:
                        ...
                        if mtick not in ticketlist:
                            message = (
                                "\n- File has titlerights!!! Missing ticket: " + mtick
                            )
                            print(message)
                            feed += message + "\n"
                            verdict = False
    ...
    def verify_sig(self, feed, tmpfolder, cnmt="check"):
        ...
        for f in self:
            if type(f) == Nca and f.header.contentType != Type.Content.META:
                hlisthash = False
                message = str(f.header.titleId) + " - " + str(f.header.contentType)
                print(message)
                feed += message + "\n"
                verify, origheader, ncaname, feed, origkg, tr, tkey, iGC = f.verify(
                    feed
                )
            ...
            if str(f._path).endswith(".ncz"):
                ...
                ncz = Nca(f)
                ncz._path = f._path
                message = str(ncz.header.titleId) + " - " + str(ncz.header.contentType)
                print(message)
                feed += message + "\n"
                verify, origheader, ncaname, feed, origkg, tr, tkey, iGC = ncz.verify(
                    feed
                )
                # headerlist.append([ncaname,origheader,hlisthash])
                headerlist.append([ncaname, origheader, hlisthash, tr, tkey, iGC])
            ...
            if type(f) == Nca and f.header.contentType == Type.Content.META:
                ...
                if verify == False and cnmt != "check":
                    ...
                    if str(sha0.hexdigest())[0:32] == str(f._path)[0:32]:
                        print(
                            "        * File was rehashed, will check validity after restoration"
                        )
                    hlisthash = "patched"
                if verify == False and cnmt == "check":
                        ...
                        if topkg >= origkg:
                            ...
                            for verNumber in kg:
                                ...
                                (
                                    ...
                                    tkey,
                                    iGC,
                                ) = fp.verify(feed, targetkg, rsv_endcheck, progress, t)
                                fp.close()
                                t.update(1)
                                if verify == True:
                                    t.close()
                                    ...
                                    print(message)
                                    feed += message + "\n"
                                    message = tabs + "* " + "THE CNMT FILE IS CORRECT"
                                    print(message)
                                    feed += message + "\n"
                                    hlisthash = True
                                    cnmtdidverify = True
        ...
        if str(self.path).endswith(".nsz"):
            ...
            token = "NSP"
        if verdict == False:
            message = ("VERDICT: {} FILE COULD'VE BEEN TAMPERED WITH").format(token)
            print(message)
            feed += message + "\n"
        if verdict == True:
            message = ("VERDICT: {}  FILE IS SAFE").format(token)
            print(message)
            feed += message + "\n"
    ...
    def verify_hash_nca(self, buffer, headerlist, didverify, feed):
        ...
        if str(self.path).endswith(".nsz"):
            ...
            token = "NSP"
        if verdict == False:
            message = ("VERDICT: {} FILE IS CORRUPT").format(token)
            print(message)
            feed += message + "\n"
        if verdict == True:
            message = ("VERDICT: {} FILE IS CORRECT").format(token)
            print(message)
            feed += message + "\n"
        return verdict, feed
        ...
        for f in self:
            if str(f._path) == nca:
                ...
                if type(f) == Fs.Nca:
                    for fs in f.sectionFilesystems:
                        if (
                            fs.fsType == Type.Fs.ROMFS
                            and fs.cryptoType == Type.Crypto.CTR
                            or f.header.contentType == Type.Content.MANUAL
                            or f.header.contentType == Type.Content.DATA
                        ):
                            f.seek(0)
                            ncaHeader = f.read(0x400)
    ...
    def verify_key(self, nca, ticket, userkey=False):
        ...
        if userkey == False:
            ...
            titleKeyDec = Keys.decryptTitleKey(
                encKey, Keys.getMasterKeyIndex(masterKeyRev)
            )
            verticket = True
                    ...
                    for fs in f.sectionFilesystems:
                        ...
                        if (
                            ...
                            if sectionHeaderBlock[8:12] == b"IVFC":
                                ...
                                if hx(sectionHeaderBlock[0xC8 : 0xC8 + 0x20]).decode(
                                    "utf-8"
                                ) == str(sha256(data).hexdigest()):
                                    return True
                                else:
                                    return False
                            else:
                        ...
                        if (
                            fs.fsType == Type.Fs.ROMFS
                            and fs.cryptoType == Type.Crypto.BKTR
                            and str(f.header.contentType) == "Content.PROGRAM"
                        ):
                            f.seek(0)
                            ncaHeader = NcaHeader()
                            ncaHeader.open(
        ...
        for i in range(len(files_list)):
            ...
            if filepath.endswith(".cnmt.nca"):
                (
                    ...
                    DeltaSize,
                    ncadata,
                ) = self.get_data_from_cnmt(filepath)
        ...
        for i in range(len(headerlist)):
            ...
            if entry[1] != False and entry[4] != False:
                ...
                if rights not in rightslist:
                    ...
                    ticketlist.append(gentik)
                    certlist.append(gencert)
                    tikname = str(rights.lower()) + ".tik"
                    certname = str(rights.lower()) + ".cert"
                    files.append(tikname)
                    files.append(certname)
                    filesizes.append(len(gentik))
                    filesizes.append(len(gencert))
        if output_type == "nsp":
            ...
            (
                ...
                norm_multiplier,
                sec_multiplier,
            ) = sq_tools.get_xciheader(files, filesizes, sec_hashlist)
    ...
    def return_DBdict(self):
        ...
        # Loop through self to find NCA files of type Content.META
        for nca in self:
            if isinstance(nca, Nca):
                # Log the content type of each NCA
                # print(f"Found NCA: {nca._path}, contentType: {nca.header.contentType}")
                if nca.header.contentType == 1:
                    cnmtnames.append(str(nca._path))

        content_number = len(cnmtnames)

        # Handle cases based on number of content files
        if content_number > 1:
            try:
                file, mcstring, mGame = self.choosecnmt(cnmtnames)
                DBdict = self.getDBdict(
                    file, content_number=content_number, mcstring=mcstring, mGame=mGame
                )
            except Exception as e:
                # print(f"Error in processing multiple content files: {e}")
                raise
        elif content_number == 1:
            try:
                DBdict = self.getDBdict(cnmtnames[0], content_number=content_number)
            except Exception as e:
                # print(f"Error in processing single content file: {e}")
                raise
        else:
            # print("No content files found. cnmtnames is empty.")
            raise IndexError("No content files found. cnmtnames is empty.")
    ...
    def getDBdict(
        ...
        Print.info(f"Starting getDBdict for cnmtname: {cnmtname}")
        DBdict = {}

        try:
            # Extract key data from the cnmt file
            (
                titleid,
                titleversion,
                base_ID,
                ...
                DeltaSize,
                ncadata,
            ) = self.get_data_from_cnmt(cnmtname)
            Print.info(
                f"Extracted data from cnmt: titleid={titleid}, ctype={ctype}, InstalledSize={Installedsize}"
            )
        except Exception as e:
            Print.error(f"Error extracting data from cnmt: {e}")
            return None

        try:
            # Attempt to get additional names and languages
            try:
                sqname, sqeditor, SupLg, ctype = self.DB_get_names(ctype, ncadata)
                Print.info(
                    f"Retrieved names from DB: sqname={sqname}, sqeditor={sqeditor}, languages={SupLg}"
                )
            except Exception as e:
                Print.warning(
                    f"DB_get_names failed, trying DB_get_names_from_nutdb: {e}"
                )
                sqname, sqeditor, SupLg = self.DB_get_names_from_nutdb(titleid)
                Print.info(
                    f"Retrieved names from nutdb: sqname={sqname}, sqeditor={sqeditor}"
                )

            # Get additional data for non-DLC content
            if ctype != "DLC":
                nacpdict = self.get_data_from_nacp(ncadata)
                Print.info(f"NACP data retrieved: {nacpdict}")

            titlekey, dectkey = self.db_get_titlekey(rightsId, keygeneration)
            Print.info(f"Title keys retrieved: titlekey={titlekey}, dectkey={dectkey}")

            # Fill in DBdict with the retrieved data
            DBdict["id"] = titleid
            ...
            if ctype != "DLC":
                try:
                    if (
                        nacpdict["StartupUserAccount"]
                        == "RequiredWithNetworkServiceAccountAvailable"
                        or nacpdict["RequiredNetworkServiceLicenseOnLaunch"] != "None"
                    ):
                        DBdict["linkedAccRequired"] = "True"
                    else:
                        DBdict["linkedAccRequired"] = "False"
                    DBdict["dispversion"] = nacpdict["BuildNumber"]
                    # ... more assignments from nacpdict ...
                    Print.info(f"NACP data processed successfully")
                except Exception as e:
                    Print.warning(f"Error processing NACP data: {e}")
            ...
            # Retrieve additional content data
            if ctype in ["GAME", "DLC", "DEMO"]:
                data = nutdb.get_content_data(titleid, trans)
                regions = nutdb.get_contenregions(titleid)
            else:
                data = nutdb.get_content_data(base_ID, trans)
            ...
            # Log the retrieved content data
            Print.info(
                f"Content data retrieved: nsuId={nsuId}, worldreleasedate={worldreleasedate}, genretags={genretags}"
            )

            # Assign additional data to DBdict
            if nsuId:
                DBdict["nsuId"] = nsuId
            if genretags:
                DBdict["genretags"] = genretags
            # ... continue to assign other data ...

            # Get metascores
            metascore, userscore, openscore = nutdb.get_metascores(titleid)
            DBdict["metascore"] = "-" if not metascore else metascore
            DBdict["userscore"] = "-" if not userscore else userscore
            DBdict["openscore"] = "-" if not openscore else openscore

            Print.info("getDBdict completed successfully")
            return DBdict

        except Exception as e:
            Print.error(f"Error in getDBdict: {e}")
            return None
    ...
    def get_data_from_cnmt(self, cnmtname):
        print(f"Starting get_data_from_cnmt for cnmtname: {cnmtname}")
        for nca in self:
            print(f"Processing NCA: {nca}")

            if type(nca) == Nca:
                print(
                    f"NCA is of type Nca, checking contentType: {nca.header.contentType}"
                )

                if nca.header.contentType == 1:
                    print(f"NCA contentType is 'Content.META'. Path: {nca._path}")

                    if str(nca._path) == cnmtname:
                        print("Path matches cnmtname, proceeding with extraction.")
                        try:
                            crypto1 = nca.header.getCryptoType()
                            crypto2 = nca.header.getCryptoType2()
                            print(f"crypto1: {crypto1}, crypto2: {crypto2}")

                            keygeneration = max(crypto1, crypto2)
                            print(f"Determined keygeneration: {keygeneration}")

                            for f in nca:
                                for cnmt in f:
                                    print("Found CNMT in NCA, processing...")
                                    nca.rewind()
                                    f.rewind()
                                    cnmt.rewind()
                                    ...
                                    titleid2 = titleid2[2:-1]
                                    titleid = titleid2.upper()

                                    print(f"titleid: {titleid}, titleid2: {titleid2}")

                                    titleversion = cnmt.read(0x4)
                                    titleversion = str(
                                        int.from_bytes(titleversion, byteorder="little")
                                    )
                                    print(f"titleversion: {titleversion}")

                                    type_n = cnmt.read(0x1)
                                    print(f"type_n: {type_n}")
                                    ...
                                    meta_entries = cnmt.readInt16()
                                    print(
                                        f"offset: {offset}, content_entries: {content_entries}, meta_entries: {meta_entries}"
                                    )
                                    ...
                                    base_ID = (
                                        str(hx(base_ID.to_bytes(8, byteorder="big")))[
                                            2:-1
                                        ]
                                    ).upper()
                                    ...
                                    length_of_emeta = cnmt.readInt32()
                                    print(
                                        f"min_sversion: {min_sversion}, length_of_emeta: {length_of_emeta}"
                                    )

                                    content_type_cnmt = str(cnmt._path)
                                    content_type_cnmt = content_type_cnmt[:-22]
                                    print(f"content_type_cnmt: {content_type_cnmt}")

                                    # Determine RSV and RGV
                                    if content_type_cnmt != "AddOnContent":
                                        RSV = str(min_sversion)
                                        RSV = sq_tools.getFWRangeRSV(int(RSV))
                                        RGV = 0
                                    else:
                                        RSV_rq_min = sq_tools.getMinRSV(
                                            keygeneration, 0
                                        )
                                        RSV = sq_tools.getFWRangeRSV(int(RSV_rq_min))
                                        RGV = str(min_sversion)
                                    print(f"RSV: {RSV}, RGV: {RGV}")

                                    # Determine ctype
                                    type_n_hex = str(hx(type_n))
                                    ctype = {
                                        "b'1'": "SystemProgram",
                                        "b'2'": "SystemData",
                                        "b'3'": "SystemUpdate",
                                        "b'4'": "BootImagePackage",
                                        "b'5'": "BootImagePackageSafe",
                                        "b'80'": "GAME",
                                        "b'81'": "UPDATE",
                                        "b'82'": "DLC",
                                        "b'83'": "Delta",
                                    }.get(type_n_hex, "Unknown")
                                    print(f"ctype: {ctype}")

                                    # Additional data extraction
                                    metasdkversion = nca.get_sdkversion()
                                    programSDKversion, dataSDKversion = (
                                        self.getsdkvertit(titleid2)
                                    )
                                    exesdkversion = (
                                        dataSDKversion
                                        if content_type_cnmt == "AddOnContent"
                                        else programSDKversion
                                    )
                                    print(
                                        f"metasdkversion: {metasdkversion}, exesdkversion: {exesdkversion}"
                                    )
                                    ...
                                    print(f"cnmtdata: {cnmtdata}")

                                    rightsId = (
                                        titleid + "000000000000000" + str(crypto2)
                                    )
                                    print(f"rightsId: {rightsId}")
                            ...
                            print(f"Error extracting data from cnmt: {e}")

        print("No matching CNMT data found, returning None.")
        return None
                ...
                if str(nca.header.contentType) == "Content.CONTROL":
                    if target == str(nca._path):
                        ...
                        for f in nca:
                            f.seek(offset)
                            nacp = Nacp()
                            dict["RegionalNames"], dict["RegionalEditors"] = (
                                nacp.get_NameandPub(f.read(0x300 * 15))
                            )
                            f.seek(offset + 0x3000)
                            dict["ISBN"] = nacp.get_Isbn(f.read(0x24))
                            f.seek(offset + 0x3025)
                            dict["StartupUserAccount"] = nacp.get_StartupUserAccount(
                                f.readInt8("little")
                            )
                            dict["UserAccountSwitchLock"] = (
                                nacp.get_UserAccountSwitchLock(f.readInt8("little"))
                            )
                            dict["AddOnContentRegistrationType"] = (
                                nacp.get_AddOnContentRegistrationType(
                                    f.readInt8("little")
                                )
                            )
                            dict["ctype"] = nacp.get_ContentType(f.readInt8("little"))
                            dict["ParentalControl"] = nacp.get_ParentalControl(
                                f.readInt8("little")
                            )
                            dict["ScreenshotsEnabled"] = nacp.get_Screenshot(
                                f.readInt8("little")
                            )
                            dict["VideocaptureEnabled"] = nacp.get_VideoCapture(
                                f.readInt8("little")
                            )
                            dict["dataLossConfirmation"] = (
                                nacp.get_dataLossConfirmation(f.readInt8("little"))
                            )
                            dict["PlayLogPolicy"] = nacp.get_PlayLogPolicy(
                                f.readInt8("little")
                            )
                            dict["PresenceGroupId"] = nacp.get_PresenceGroupId(
                                f.readInt64("little")
                            )
                            f.seek(offset + 0x3040)
                            ...
                            f.seek(offset + 0x3060)
                            try:
                                dict["BuildNumber"] = nacp.get_DisplayVersion(
                                    f.read(0xF)
                                )
                                f.seek(offset + 0x3070)
                                dict["AddOnContentBaseId"] = (
                                    nacp.get_AddOnContentBaseId(f.readInt64("little"))
                                )
                                f.seek(offset + 0x3078)
                                dict["SaveDataOwnerId"] = nacp.get_SaveDataOwnerId(
                                    f.readInt64("little")
                                )
                                f.seek(offset + 0x3080)
                                dict["UserAccountSaveDataSize"] = (
                                    nacp.get_UserAccountSaveDataSize(
                                        f.readInt64("little")
                                    )
                                )
                                f.seek(offset + 0x3088)
                                dict["UserAccountSaveDataJournalSize"] = (
                                    nacp.get_UserAccountSaveDataJournalSize(
                                        f.readInt64("little")
                                    )
                                )
                                f.seek(offset + 0x3090)
                                dict["DeviceSaveDataSize"] = (
                                    nacp.get_DeviceSaveDataSize(f.readInt64("little"))
                                )
                                f.seek(offset + 0x3098)
                                dict["DeviceSaveDataJournalSize"] = (
                                    nacp.get_DeviceSaveDataJournalSize(
                                        f.readInt64("little")
                                    )
                                )
                                f.seek(offset + 0x30A0)
                                dict["BcatDeliveryCacheStorageSize"] = (
                                    nacp.get_BcatDeliveryCacheStorageSize(
                                        f.readInt64("little")
                                    )
                                )
                                f.seek(offset + 0x30A8)
                                dict["ApplicationErrorCodeCategory"] = (
                                    nacp.get_ApplicationErrorCodeCategory(f.read(0x07))
                                )
                                f.seek(offset + 0x30B0)
                                dict["LocalCommunicationId"] = (
                                    nacp.get_LocalCommunicationId(f.readInt64("little"))
                                )
                                f.seek(offset + 0x30F0)
                                dict["LogoType"] = nacp.get_LogoType(
                                    f.readInt8("little")
                                )
                                dict["LogoHandling"] = nacp.get_LogoHandling(
                                    f.readInt8("little")
                                )
                                dict["RuntimeAddOnContentInstall"] = (
                                    nacp.get_RuntimeAddOnContentInstall(
                                        f.readInt8("little")
                                    )
                                )
                                dict["CrashReport"] = nacp.get_CrashReport(
                                    f.readInt8("little")
                                )
                                dict["Hdcp"] = nacp.get_Hdcp(f.readInt8("little"))
                                dict["SeedForPseudoDeviceId"] = (
                                    nacp.get_SeedForPseudoDeviceId(
                                        f.readInt64("little")
                                    )
                                )
                                f.seek(offset + 0x3100)
                                dict["BcatPassphrase"] = nacp.get_BcatPassphrase(
                                    f.read(0x40)
                                )
                                f.seek(offset + 0x3148)
                                dict["UserAccountSaveDataSizeMax"] = (
                                    nacp.par_UserAccountSaveDataSizeMax(
                                        f.readInt64("little")
                                    )
                                )
                                f.seek(offset + 0x3150)
                                dict["UserAccountSaveDataJournalSizeMax"] = (
                                    nacp.par_UserAccountSaveDataJournalSizeMax(
                                        f.readInt64("little")
                                    )
                                )
                                f.seek(offset + 0x3158)
                                dict["DeviceSaveDataSizeMax"] = (
                                    nacp.get_DeviceSaveDataSizeMax(
                                        f.readInt64("little")
                                    )
                                )
                                f.seek(offset + 0x3160)
                                dict["DeviceSaveDataJournalSizeMax"] = (
                                    nacp.get_DeviceSaveDataJournalSizeMax(
                                        f.readInt64("little")
                                    )
                                )
                                f.seek(offset + 0x3168)
                                dict["TemporaryStorageSize"] = (
                                    nacp.get_TemporaryStorageSize(f.readInt64("little"))
                                )
                                dict["CacheStorageSize"] = nacp.get_CacheStorageSize(
                                    f.readInt64("little")
                                )
                                f.seek(offset + 0x3178)
                                dict["CacheStorageJournalSize"] = (
                                    nacp.get_CacheStorageJournalSize(
                                        f.readInt64("little")
                                    )
                                )
                                dict["CacheStorageDataAndJournalSizeMax"] = (
                                    nacp.get_CacheStorageDataAndJournalSizeMax(
                                        f.readInt64("little")
                                    )
                                )
                                f.seek(offset + 0x3188)
                                dict["CacheStorageIndexMax"] = (
                                    nacp.get_CacheStorageIndexMax(f.readInt64("little"))
                                )
                                dict["PlayLogQueryableApplicationId"] = (
                                    nacp.get_PlayLogQueryableApplicationId(
                                        f.readInt64("little")
                                    )
                                )
                                f.seek(offset + 0x3210)
                                dict["PlayLogQueryCapability"] = (
                                    nacp.get_PlayLogQueryCapability(
                                        f.readInt8("little")
                                    )
                                )
                                dict["Repair"] = nacp.get_Repair(f.readInt8("little"))
                                dict["ProgramIndex"] = nacp.get_ProgramIndex(
                                    f.readInt8("little")
                                )
                                dict["RequiredNetworkServiceLicenseOnLaunch"] = (
                                    nacp.get_RequiredNetworkServiceLicenseOnLaunch(
                                        f.readInt8("little")
                                    )
                                )
                            except:
        ...
        for ticket in self:
            if type(ticket) == Ticket:
                ...
                if str(rightsId).lower() == str(tikrights).lower():
                    ...
                    deckey = Keys.decryptTitleKey(
                        ticket.getTitleKeyBlock().to_bytes(16, byteorder="big"),
                        Keys.getMasterKeyIndex(int(keygeneration)),
                    )
                    ...
                    try:
                        ...
                        try:
                            with open(str(self._path), "rb") as f:
                                ...
                                nca3 = NCA3(
                                    inmemoryfile, int(0), str(nca._path), tk, buffer
                                )
                                for dat in self.open_all_dat(nca3):
                                    checksum = sha1(dat.read()).digest()
                                    if checksum not in checksums:
    ...
    def decompress_direct(self, output, buffer=65536, delta=False, xml_gen=False):
        ...
        for file in files:
            if file.endswith("cnmt.nca"):
                ...
                for nca in self:
                    if str(nca._path)[:-1] == file[:-1] and not str(nca._path).endswith(
                        ".ncz"
                    ):
                        t.write("- Appending {}".format(str(nca._path)))
                        o = open(output, "ab+")
                        ...
                        for data in iter(lambda: nca.read(int(buffer)), ""):
                            ...
                            if not data:
                                o.close()
                                break
                    elif str(nca._path)[:-1] == file[:-1] and str(nca._path).endswith(
                        "ncz"
                    ):
                        # print(nca._path)
                        nca.rewind()
                        ...
                        with open(output, "rb+") as o:
                            ...
                            curr_off = o.tell()
                            t.write(
                                "- Appending decompressed {}".format(str(nca._path))
                            )
                            t.write("  Writing nca header")
                            o.write(header)
                            t.update(len(header))
    ...
    def verify_ncz(self, target):
        ...
        for file in files:
            if file.endswith("nca"):
                for nca in self:
                    if str(nca._path)[:-1] == file[:-1] and str(nca._path).endswith(
                        "ncz"
                    ):
                        # print(nca._path)
                        nca.rewind()
