...
def get_path_parameters(route):
    ...
    if route == "drive":
        ...
        tid1 = list()
        tid1 = [pos for pos, char in enumerate(route) if char == "/"]
        tid2 = list()
        tid2 = [pos for pos, char in enumerate(route) if char == "\\"]
        positions = list(set(tid1 + tid2))
        positions.sort()
...
class location:
    ...
    def get_name(self, ID=None, token_name=None):
        ...
        if self.isinTD == False:
            result = (
                drive_service.files().get(fileId=ID, fields="name,mimeType").execute()
            )
        else:
            result = (
                drive_service.files()
    ...
    def get_hash(self, ID=None, token_name=None):
        ...
        if self.isinTD == False:
            result = (
                drive_service.files().get(fileId=ID, fields="md5Checksum").execute()
            )
        else:
            result = (
                drive_service.files()
                .get(fileId=ID, fields="md5Checksum", supportsAllDrives=True)
                .execute()
            )
    ...
    def get_size(self, ID=None, token_name=None):
        ...
        try:
            if self.isinTD == False:
                ...
                result = (
                    drive_service.files()
                    .get(fileId=ID, fields="size", supportsAllDrives=True)
                    .execute()
                )
            size = result["size"]
            self.size = size
...
def token_picker():
    ...
    if len(names) > 1:
        title = (
            "Pick an account (press SPACE\\RIGHT to mark\\unmark, ENTER to continue): "
        )
        options = names
...
def folder_walker(showfiles=False, Print=False):
    ...
    while True:
        folder = search_folder(
            path, TD=TD, mime="folders", pickmode="single", Print=False
        )
        if folder == False:
            break
        if folder == "EXIT":
                ...
                while True:
                    if root != "root":
                        results = (
                            drive_service.files()
                            .list(
                                q="mimeType!='application/vnd.google-apps.folder' and '{}' in parents{}".format(
                                    remote.ID, filter
                                ),
                                pageSize=pagesize,
    ...
    try:
        ...
        if makecopy != True:
            file = (
                ...
                .update(fileId=file_id, addParents=FolderID, fields="id,parents,name")
                .execute()
            )
            name = file.get("name")
        else:
            ...
            file = (
                remote.drive_service.files()
                .copy(fileId=file_id, body=newfile, fields="id, parents")
                .execute()
            )
...
def delete_from_drive(filepath=None, url=None, lkID=None, TD=None):
    try:
        ...
        file = (
            remote.drive_service.files().get(fileId=file_id, fields="parents").execute()
        )
        ...
        file = (
            remote.drive_service.files()
            .update(
                fileId=file_id, removeParents=previous_parents, fields="name,id,parents"
            )
            .execute()
        )
...
def move(origin, destiny, originTD, destinyTD):
    ...
    folderID = Dremote.ID
    # print(folderID)
    file = Oremote.drive_service.files().get(fileId=fileId, fields="parents").execute()
    previous_parents = ",".join(file.get("parents"))
    file = (
        Dremote.drive_service.files()
        .update(
            fileId=fileId,
...
def clean_name(name):
    ...
    name = re.sub(r"[àâá@äå]", "a", name)
    name = re.sub(r"[ÀÂÁÄÅ]", "A", name)
    name = re.sub(r"[èêéë]", "e", name)
    name = re.sub(r"[ÈÊÉË]", "E", name)
    name = re.sub(r"[ìîíï]", "i", name)
    name = re.sub(r"[ÌÎÍÏ]", "I", name)
    name = re.sub(r"[òôóöø]", "o", name)
    name = re.sub(r"[ÒÔÓÖØ]", "O", name)
    name = re.sub(r"[ùûúü]", "u", name)
    name = re.sub(r"[ÙÛÚÜ]", "U", name)
