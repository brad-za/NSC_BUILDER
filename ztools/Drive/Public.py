...
class location:
    def __init__(self, url_origin=None, ID=None, url=None, doPrint=True):
        ...
        self.readable, self.name, self.url, self.response, self.sess = is_readable(
            self.url, self.ID, is_download_link, self.sess
        )
        self.get_size()
        self.rewind()
        if (
            (self.name.lower()).endswith(".nsp")
            or (self.name.lower()).endswith(".nsx")
            or (self.name.lower()).endswith(".nsz")
        ):
            self.ftype = "nsp"
        elif (self.name.lower()).endswith(".xci") or (self.name.lower()).endswith(
            ".xcz"
        ):
            self.ftype = "xci"
