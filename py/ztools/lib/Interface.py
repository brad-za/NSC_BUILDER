...
def About(noconsole=False):
    ...
    print(
        "Cheats and Eshop information from nutdb and http://tinfoil.io                         "
    )
    print(
        "------------------------------------------------------------------------------------- "
    )
...
def backend_test_function():
    print("Backend function called", file=sys.stderr)
    sys.stderr.flush()
    result = "Simple test string"
...
@eel.expose
def backend_receive_data(data):
    print(f"Backend: Received data from frontend: {data}")  # Log received data
    sys.stdout.flush()
...
globalremote = None
globalocalpath = None
gl_current_local_lib = "all"
gl_current_remote_lib = "all"
threads = []
...
def html_feed(feed="", style=1, message=""):
    ...
    if style == 7:
        ...
        return feed
    if style == 8:
        feed += '<p style="margin-bottom: 2px;margin-top: 2px;font-size: 1.8vh;">{}</p>'.format(
            message
        )
        return feed
    if style == 9:
        feed += '<li style="margin-bottom: 2px;margin-top: 3px;list-style-type:none;"><strong>{} </strong><span>{}</span></li>'.format(
            message[0], message[1]
        )
        return feed
    if style == 10:
        feed += '<li style="margin-bottom: 2px;margin-top: 3px;list-style-type:none;"><strong style=border-style: solid;>{} </strong><span>{}</span></li>'.format(
            message[0], message[1]
        )
        return feed
...
def get_screen_gallery(banner, screenlist):
    try:
        ret = '<div class="grid"><div class="row"><div class="img-container thumbnail" style="width: auto;max-height:auto; margin-left: auto; margin-right: auto; display: block;" onclick="showimg()"><img src="{}" id="mainpicture" style="width: auto;max-height:63vh;"></div></div></div><div class="row">'.format(
            banner
        )
        banner1 = "'{}'".format(banner)
        ret += '<div class="cell" style="width: auto;max-height:5vh;"><div class="img-container thumbnail" onclick="setmainimage({})"><img src="{}"></div></div>'.format(
            banner1, banner
        )
        ...
        if isinstance(screenlist, list):
            ...
            for x in screenlist:
                x1 = "'{}'".format(x)
                ret += '<div class="cell"><div class="img-container thumbnail"  onclick="setmainimage({})"><img src="{}" id="gallerypicture{}"></div></div>'.format(
                    x1, x, i
                )
                i += 1
        if len(screenlist) < 5:
            ...
            for x in range(number):
                ...
                ret += '<div class="cell"><div class="img-container thumbnail"><img src="{}"></div></div>'.format(
                    x
                )
...
def format_file_tree(baselist, updlist, dlclist, titleid):
    ...
    if len(baselist) == 0:
        message = ["TitleID:", (titleid.upper() + " <strong>Version:</strong> " + "0")]
        feed = html_feed(feed, 3, message)
    else:
        for i in range(len(baselist)):
            p_ = baselist[i]
            message = [
                "TitleID: ",
                (p_[0].upper() + " <strong>Version:</strong> " + p_[1]),
            ]
            feed = html_feed(feed, 3, message)
    feed += "</ul>"
    ...
    try:
        ...
        if not library.lower() == "all":
            ...
            for it in sorted(sr.keys()):
                ...
                html += '<li style="margin-bottom: 2px;margin-top: 3px" onclick="start_from_library({})"><span id="{}" style="display:none">{}</span>{}<strong>{}</strong></li>'.format(
                    var, var, item, type, item2
                )
            html += "</ul>"
        else:
            results = []
...
def get_libraries():
    try:
        ...
        if db == False:
            htmlcode = '<select class="bg-cobalt fg-white" data-role="select" style="width:25vw" id="_local_lib_select_">'
            htmlcode += '<option value="{}">{}</option>'.format(
    ...
    try:
        ...
        if not library.lower() == "all":
            ...
            for it in sorted(sr.keys()):
                ...
                html += '<li style="margin-bottom: 2px;margin-top: 3px" onclick="start_from_remote_library({})"><span id="{}" style="display:none">{}</span>{}<strong>{}</strong></li>'.format(
                    var, var, item, type, item2
                )
            html += "</ul>"
        else:
            ...
            for entry in db:
                ...
                print("* Searching library {}".format(entry))
                sys.stdout.flush()
                response = DrivePrivate.search_folder(
                    path, TD=TD, filter=value, Pick=False
                )
                if response != False:
                    results += response
            send_results = []
...
def getfname():
    ...
    filename = filedialog.askopenfilename(
        filetypes=[
            ...
            ("nsp", "*.ns0"),
            ("nsp", "*.nsx"),
            ("Compressed files", "*.nsz"),
            ("Compressed files", "*.xcz"),
            ("Uncompressed files", "*.nsp"),
            ("Uncompressed files", "*.nsx"),
            ("Uncompressed files", "*.xci"),
            ("Uncompressed files", "*.xc0"),
            ("Uncompressed files", "*.ns0"),
            ("Uncompressed files", "00"),
            ("Split Files", "*.ns0"),
            ("Split Files", "*.xc0"),
            ("Split Files", "00"),
        ]
    ...
    if os.path.exists(cache_lib_file):
        ...
        if lib != None:
            ...
            if is_download_link:
                ...
                if remote.name == None:
                    name = DrivePrivate.add_to_drive(
                        url=filename, filepath=libpath, makecopy=True, TD=TD
                    )
                    filename = ("{}/{}").format(libpath, name)
                    ID, name, type, size, md5, remote = DrivePrivate.get_Data(
                        filename, TD=TD, Print=False
                    )
                else:
                    filename = testpath
                globalremote = remote
...
def showicon(filename):
    ...
    try:
        if (
            ...
            or filename.endswith(".nsx")
            or filename.endswith(".nsz")
        ):
            ...
            filename.endswith(".xc0")
            or filename.endswith(".ns0")
            or filename.endswith("00")
        ):
...
def showicon_remote(filename):
    ...
    if globalpath != filename:
        if not filename.startswith("http"):
            ...
            globalpath = filename
            remote = DrivePublic.location(filename)
...
def getinfo(filename, remotelocation=False):
    ...
    if remotelocation == False:
        ...
        try:
            if (
                remotename.endswith(".nsp")
                or remotename.endswith(".nsx")
                or remotename.endswith(".nsz")
            ):
                send_.append("Eshop")
...
def getfiletree(ID):
    ...
    try:
        ...
        print(baselist, updlist, dlclist, ID)
...
def getcheats(ID):
    ...
    try:
        ...
        for i in range(len(cheatID_list)):
            ...
            version = data[1]
            buildid = data[2]
            message = [str(titleid + " v" + version + ". " + "BuildID: " + buildid), ""]
            feed = html_feed(feed, 3, message)
            cheats = data[3]
...
def server(
    ...
    if noconsole == True:
        sys.stdout = open(
            os.path.join(debug_folder, "log_{}.txt".format(str(port))), "w"
        )
...
def start(
    ...
    try:
        if browserpath == "default":
            ...
            browserpath != "auto"
            and os.path.exists(browserpath)
            and not browserpath.endswith(".ink")
        ):
            ...
            try:
                ...
                print(
                    "Chrome wasn't detected. Launched using Windows Edge with limited compatibility"
                )
                sys.stdout.flush()
