...
class ChromeNsp(Pfs0):
                        ...
                        try:
                            ...
                            for fs in nca.sectionFilesystems:
                                ...
                                if (
                                    ...
                                    head = data[0:4]
                                    n_files = data[4:8]
                                    n_files = int.from_bytes(
                                        n_files, byteorder="little"
                                    )
                                    st_size = data[8:12]
                                    st_size = int.from_bytes(
                                        st_size, byteorder="little"
                                    )
                                    junk = data[12:16]
                                    ...
                                    if head != b"PFS0":
                                        feed = self.html_feed(
                                            feed,
                                            2,
                                            message=str("- Error decrypting npdm"),
                                        )
                                        continue
                                    # print(str(n_files))
                                    # print(str(st_size))
                                ...
                                for i in range(len(files_list)):
                                    if files_list[i][0] == "main":
                                        ...
                                        if magic == b"NSO0":
                                            ...
                                            BuildID16 = (str(hx(data[:16])).upper())[
                                                2:-1
                                            ]
                                            iscorrect = True
                                        break
        ...
        for nca in self:
            if type(nca) == Nca:
                if str(nca.header.contentType) == "Content.PROGRAM":
                    ...
                    titleKeyEnc = Keys.encryptTitleKey(
                        decKeyBlock, Keys.getMasterKeyIndex(masterKeyRev)
                    )
    ...
    def html_feed(self, feed="", style=1, message=""):
        ...
        if message == None:
            message = ""
        if style == 1:
            feed += '<p style="font-size: 14px; justify;text-justify: inter-word;"><strong>{}</strong></p>'.format(
                message
            )
            return feed
        if style == 2:
            feed += '<p style="margin-bottom: 2px;margin-top: 2px;font-size: 2vh"><strong>{}</strong></p>'.format(
                message
            )
            return feed
        if style == 3:
            feed += '<li style="margin-bottom: 2px;margin-top: 3px"><strong>{} </strong><span>{}</span></li>'.format(
                message[0], message[1]
            )
            return feed
        if style == 4:
            feed += '<li style="margin-bottom: 2px;margin-top: 3px"><strong>{} </strong><span>{}. </span><strong>{} </strong><strong>{}</strong></li>'.format(
                message[0], message[1], message[2], message[3]
            )
            return feed
        if style == 5:
            feed += '<p style="margin-bottom: 2px;margin-top: 2px;font-size: 2vh;text-indent:5vh";><strong style="text-indent:5vh;">{}</strong></p>'.format(
                message
            )
            return feed
        if style == 6:
            feed += '<p style="margin-bottom: 2px;margin-top: 2px;font-size: 1.8vh;";><strong>{}</strong></p>'.format(
                message
            )
            return feed
        if style == 7:
        ...
        if style == 8:
            feed += '<li style="margin-bottom: 2px;margin-top: 3px"><strong>[{}] {} </strong><span>{}. </span><strong>{} </strong><strong>{}</strong></li>'.format(
                message[3], message[0], message[1], message[4], message[5]
            )
            return feed
                    ...
                    for f in nca:
                        for cnmt in f:
                            ...
                            if content_type_cnmt != "AddOnContent":
                                ...
                                data = "{} -> {} ({})".format(
                                    version, content_type_cnmt, v_number
                                )
                                message = ["Version:", data]
                                feed = self.html_feed(feed, 3, message=message)
                            if content_type_cnmt == "AddOnContent":
                                if tit_name != "DLC":
                                    message = ["Name:", tit_name]
                                    feed = self.html_feed(feed, 3, message)
                            ...
                            feed += "</ul>"
                            feed = self.html_feed(
                                feed, 2, message=str("- Required Firmware:")
                            )
                            incl_Firm = DBmodule.FWDB.detect_xci_fw(self._path, False)
                            feed += '<ul style="margin-bottom: 2px;margin-top: 3px">'
                            message = ["Included Firmware:", str(str(incl_Firm))]
                            feed = self.html_feed(feed, 3, message)
                            if content_type_cnmt == "AddOnContent":
                                if v_number == 0:
                                    message = [
                            ...
                            ncasize = 0
                            feed = self.html_feed(
                                feed, 2, message=str("- Nca files (Non Deltas):")
                            )
                            feed += '<ul style="margin-bottom: 2px;margin-top: 3px">'
                            ...
                            feed += "</ul>"
                            feed = self.html_feed(
                                feed, 5, message=("TOTAL SIZE: " + size_pr)
                            )
                            if self.actually_has_deltas(ncalist) == "true":
                                ...
                                for i in range(content_entries):
                                    ...
                                    if ncatype == 6:
                                        feed = self.html_feed(
                                            feed, 2, message=("- Nca files (Deltas):")
                                        )
                                        feed += '<ul style="margin-bottom: 2px;margin-top: 3px">'
                                        break
                                cnmt.rewind()
                            ...
                            if self.actually_has_other(titleid2, ncalist) == "true":
                                feed = self.html_feed(
                                    feed, 2, message=("- Other types of files:")
                                )
                                feed += (
            ...
            if type(nca) == Nca:
                if filename == nca_name:
                    ...
                    if ncatype != "Meta: ":
                        ...
                        feed = self.html_feed(feed, 8, message)
                    else:
                        message = [
                            ncatype,
                            str(filename),
    ...
    def print_xml_by_title(self, ncalist, contentlist, feed=""):
        ...
        for file in self:
            if file._path.endswith(".xml"):
                ...
                xml = filename[:-4]
                if xml in ncalist:
                    message = ["XML:", str(filename), "Size:", size_pr]
                    feed = self.html_feed(feed, 4, message)
    ...
    def print_tac_by_title(self, titleid, contentlist, feed=""):
        ...
        for ticket in self:
            if type(ticket) == Ticket:
                ...
                tik = filename[:-20]
                if tik == titleid:
                    message = ["Ticket:", str(filename), "Size:", size_pr]
                    feed = self.html_feed(feed, 4, message)
                    contentlist.append(filename)
                    size2return = size + size2return
        for cert in self:
            if cert._path.endswith(".cert"):
                ...
                cert_id = filename[:-21]
                if cert_id == titleid:
                    message = ["Cert:", str(filename), "Size:", size_pr]
                    feed = self.html_feed(feed, 4, message)
                    contentlist.append(filename)
                    size2return = size + size2return
        return size2return, feed
    ...
    def print_jpg_by_title(self, ncalist, contentlist, feed=""):
        ...
        for file in self:
            if file._path.endswith(".jpg"):
                ...
                jpg = filename[:32]
                if jpg in ncalist:
                    message = ["JPG:", str(filename), "Size:", size_pr]
                    feed = self.html_feed(feed, 4, message)
                    contentlist.append(filename)
                    size2return = size + size2return
        return size2return, feed
        ...
        if list_nonlisted == "true":
            ...
            for file in self:
                ...
                if not filename in contentlist and not nczname in contentlist:
                    ...
                    message = ["OTHER:", str(filename), "Size:", size_pr]
                    feed = self.html_feed(feed, 4, message)
            ...
            if type(nca) == Nca:
                if str(nca.header.contentType) == "Content.PROGRAM":
                    ...
                    if ModuleId != "":
                        ...
                        message = [f"BuildID8:", str(BuildID8)]
                        feed = self.html_feed(feed, 3, message)
                        message = [f"BuildID:", str(ModuleId)]
                        feed = self.html_feed(feed, 3, message)
        return feed
            ...
            if type(nca) == Nca:
                if str(nca.header.contentType) == "Content.CONTROL":
                    ...
                    for f in nca:
                        ...
                        try:
                            feed = self.html_feed(
                                feed, 2, message=str("Nacp Atributes:")
                            )
                            feed += '<ul style="margin-bottom: 2px;margin-top: 3px">'
                ...
                if str(nca.header.contentType) == "Content.META":
                    ...
                    for f in nca:
                        for cnmt in f:
                            ...
                            feed = self.html_feed(feed, 3, message)
                            message = ["Cnmt Type:", (sq_tools.cnmt_type(type_n))]
                            feed = self.html_feed(feed, 3, message)
                            message = [
                                "Table offset:",
                            ...
                            if content_name != "AddOnContent":
                                ...
                                feed = self.html_feed(feed, 3, message)
                            message = ["Length of exmeta:", (str(min_sversion))]
                            feed = self.html_feed(feed, 3, message)
                            cnmt.rewind()
                            cnmt.seek(0x20 + offset)
                            for i in range(content_entries):
                                feed = self.html_feed(
                                    feed, 2, message=str("Content number " + str(i + 1))
                                )
                                vhash = cnmt.read(0x20)
                                message = ["Hash:", (str(hx(vhash))[2:-1]).upper()]
                                feed = self.html_feed(feed, 3, message)
                                NcaId = cnmt.read(0x10)
                                message = ["NcaId:", (str(hx(NcaId))[2:-1]).upper()]
                                feed = self.html_feed(feed, 3, message)
                                size = cnmt.read(0x6)
                                message = [
                                    "Size:",
                            ...
                            if length_of_emeta > 0:
                                ...
                                message = [
                                    ...
                                    (
                                        str(
                                            int.from_bytes(
                                                num_prev_cnmt, byteorder="little"
                                            )
                                        )
                                    ),
                                ]
                                ...
                                message = [
                                    ...
                                    (
                                        str(
                                            int.from_bytes(
                                                num_prev_delta, byteorder="little"
                                            )
                                        )
                                    ),
                                ]
                                ...
                                for i in range(
                                    ...
                                    feed = self.html_feed(
                                        ...
                                        2,
                                        message=str(
                                            "Previous delta records: " + str(i + 1)
                                        ),
                                    )
                                ...
                                for i in range(
                                    ...
                                    feed = self.html_feed(
                                        feed,
                                        2,
                                        message=str("Delta info: " + str(i + 1)),
                                    )
                                    oldtitleid = cnmt.readInt64()
                                    newtitleid = cnmt.readInt64()
                                    oldtitleversion = cnmt.read(0x4)
                                    ...
                                    message = [
                                        "Index1:",
                                        (str(hx(index1.to_bytes(8, byteorder="big")))),
                                    ]
                                    feed = self.html_feed(feed, 3, message)
                                    message = [
                                        "Index2:",
                                        (str(hx(index2.to_bytes(8, byteorder="big")))),
                                    ]
                                    feed = self.html_feed(feed, 3, message)
                                for i in range(
                                    int.from_bytes(
                                    ...
                                    feed = self.html_feed(
                                        ...
                                        2,
                                        message=str(
                                            "Delta application info: " + str(i + 1)
                                        ),
                                    )
                                    ...
                                    message = [
                                        "OldNcaId:",
                                        (str(hx(OldNcaId))[2:-1]).upper(),
                                    ]
                                    feed = self.html_feed(feed, 3, message)
                                    message = [
                                        "NewNcaId:",
                                        (str(hx(NewNcaId))[2:-1]).upper(),
                                    ]
                                    feed = self.html_feed(feed, 3, message)
                                    message = [
                                        "Old size:",
                                ...
                                for i in range(
                                    ...
                                    feed = self.html_feed(
                                        ...
                                        2,
                                        message=str(
                                            "Previous content records: " + str(i + 1)
                                        ),
                                    )
                                    NcaId = cnmt.read(0x10)
                                    size = cnmt.read(0x6)
    ...
    def c_nsp_direct(
        self, buffer, outfile, ofolder, fat, fx, delta, metapatch, RSV_cap, keypatch
    ):
    ...
    def c_xci_direct(
        self, buffer, outfile, ofolder, fat, delta, metapatch, RSV_cap, keypatch
    ):
        if keypatch != "false":
            try:
                keypatch = int(keypatch)
    ...
    def cd_spl_xci(self, buffer, ofile, ofolder, filelist, fat, fx):
        ...
        (
            ...
            norm_multiplier,
            sec_multiplier,
        ) = self.cd_spl_gen_xcih(contentlist)
        totSize = (
            len(xci_header)
            + len(game_info)
            + len(sig_padding)
    ...
    def verify_hash_nca(self, buffer, headerlist, didverify, feed):
        ...
        for f in self:
            if type(f) == Nca:
                ...
                feed += message + "\n"
        if verdict == False:
            message = "VERDICT: NSP FILE IS CORRUPT"
            print(message)
            feed += message + "\n"
        if verdict == True:
            message = "VERDICT: NSP FILE IS CORRECT"
            print(message)
            feed += message + "\n"
        return verdict, feed
    ...
    def getDBdict(
        ...
        if ctype == "GAME" or ctype == "DLC" or ctype == "DEMO":
            (
                ...
                video,
                shopurl,
            ) = nutdb.get_content_data(titleid, trans)
            ...
            (
                ...
                video,
                shopurl,
            ) = nutdb.get_content_data(base_ID, trans)
                    ...
                    if str(nca._path) == cnmtname:
                        ...
                        for f in nca:
                            for cnmt in f:
                                ...
                                base_ID = (
                                    str(hx(base_ID.to_bytes(8, byteorder="big")))[2:-1]
                                ).upper()
                                cnmt.rewind()
                                cnmt.seek(0x28)
                                min_sversion = cnmt.readInt32()
                    ...
                    if str(nca._path)[:-1] == file[:-1] and not str(nca._path).endswith(
                        ...
                        with open(output, "rb+") as o:
                            ...
                            for s in sections:
                                ...
                                t.write(
                                    "      Key: %s, IV: %s"
                                    % (str(hx(s.cryptoKey)), str(hx(s.cryptoCounter)))
                                )
                                i = s.offset
